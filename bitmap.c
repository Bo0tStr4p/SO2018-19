...
void BitMap_print(BitMap* bmap){
	...
	for(i=0; i < bmap->num_bits; i++){
		BitMapEntryKey key = BitMap_blockToIndex(i);
		printf("Block:%i - Bit:%i - Offset: %i - Value in BitMapEntries:%c - Status:%i\n",i, key.entry_num, key.bit_num, bmap->entries[key.entry_num], bmap->entries[key.entry_num] >> key.bit_num & 0x01 );
	}
}
